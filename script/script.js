    ...
    <div class="bg-white flex flex-col justify-between rounded-lg p-5">
       ...
       <div id="${plantInfo.id}" class="space-y-4">
         ...
         <p class="font-bold"><span class="font-bold">৳</span><span id="tree_price">${plantInfo.price}</span></p>
           ...
           <button class="bg-[#15803D] text-white text-center w-full py-3 px-5 rounded-full cursor-pointer">Add to Cart</button>
       </div>
    </div>
        `;
  });
...
const handleAddToCart = (e) =>{
    ...
    const price = e.target.parentNode.children[2].children[1].children[1].innerText
   ...
   cartArray.forEach((cartItem) =>{
    cartContainer.innerHTML += `
     <div class=" my-2 flex justify-between items-center p-3 bg-[#F0FDF4] rounded-lg">
       ...
       <div>
        <i onclick="delteCart('${cartItem.id}')" class="fa-solid fa-xmark text-[#8C8C8C]"></i>
       </div>
 </div>
    `
   })
